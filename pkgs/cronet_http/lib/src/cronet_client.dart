...
/// A public key pin.
///
/// Pins a set of public keys to a given host.
class PublicKeyPin {
  /// The host to which the public keys should be pinned.
  final String host;

  /// An array of SHA-256 hashes of public keys.
  ///
  /// Each hash is a byte array.
  final Set<List<int>> pinsSha256;

  /// Indicates whether the pinning policy should be applied to subdomains of
  /// [host].
  final bool includeSubdomains;

  /// The expiration date for the pins.
  final DateTime expirationDate;

  PublicKeyPin({
    required this.host,
    required this.pinsSha256,
...
class CronetEngine {
  ...
  ///
  /// [publicKeyPins] adds a list of public key pins.
  static CronetEngine build({
    ...
    List<PublicKeyPin>? publicKeyPins,
  }) {
    try {
      return using((arena) {
        ...
        if (quicHints != null) {
          for (final (host, port, alternativePort) in quicHints) {
            builder.addQuicHint(
              host.toJString()..releasedBy(arena),
              port,
              alternativePort,
            );
          }
        ...
        if (publicKeyPins != null) {
          for (final pin in publicKeyPins) {
            final jPins = JSet.hash(JByteArray.type)..releasedBy(arena);
            for (final pinSha256 in pin.pinsSha256) {
              final jPin = JByteArray(pinSha256.length)..releasedBy(arena);
              for (var i = 0; i < pinSha256.length; i++) {
                jPin[i] = pinSha256[i];
              }
              jPins.add(jPin);
            }
            final jDate =
                jb.Date.new$5(pin.expirationDate.millisecondsSinceEpoch)
                  ..releasedBy(arena);
            builder.addPublicKeyPins(
              pin.host.toJString()..releasedBy(arena),
              jPins,
  ...
  void startNetLogToFile(String fileName, bool logAll) {
    using((arena) {
      _engine.startNetLogToFile(
        fileName.toJString()..releasedBy(arena),
        logAll,
      );
    });
  }
      ...
      onResponseStarted: (urlRequest, responseInfo) {
        using((arena) {
          ...
          responseStream = StreamController(
            onCancel: () {
              // The user did `response.stream.cancel()`. We can just pretend
              // that the response completed normally.
          ...
          if (numRedirects <= request.maxRedirects) {
            profile?.responseData.addRedirect(
              HttpProfileRedirectData(
                ...
                // Cronet does not seem to have a way to get the method so we'd
                // have to calculate it according to the rules in RFC-7231.
                method: 'GET',
      ...
      onFailed: (urlRequest, responseInfo /* can be null */, cronetException) {
        using((arena) {
          ...
          final error = ClientException(
            'Cronet exception: ${cronetException.toString()}',
            request.url,
          );
...
class CronetClient extends BaseClient {
  ...
  Future<CronetStreamedResponse> send(BaseRequest request) async {
    ...
    if (engine._isClosed) {
      throw ClientException(
        'HTTP request failed. CronetEngine is already closed.',
        request.url,
      );
    }
    ...
    return await using((arena) async {
      ...
      headers.forEach(
        (k, v) => builder.addHeader(
          k.toJString()..releasedBy(arena),
          v.toJString()..releasedBy(arena),
        ),
      );
      ...
      if (body.isNotEmpty) {
        ...
        builder.setUploadDataProvider(
          jb.UploadDataProviders.create$2(data),
          _executor,
        );
      }
